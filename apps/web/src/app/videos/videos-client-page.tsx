...
import type { TikTokUserProfile } from "@/lib/tiktok-oauth";
...
export function VideosClientPage({
  ...
  const connectedName =
    connectionStatus.profile?.displayName ??
    connectionStatus.profile?.username ??
    "TikTok Sandbox account";
  const connectedHandle = connectionStatus.profile?.username
    ? `@${connectionStatus.profile.username}`
    : "Login Kit account";
        ...
        <Card className="rounded-lg border-0 bg-white shadow-sm ring-zinc-200">
          <CardContent>
            <div className="flex items-start gap-3">
              ...
              connectionStatus.profile?.avatarUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={connectionStatus.profile.avatarUrl}
                  alt=""
                  className="size-11 shrink-0 rounded-lg object-cover ring-1 ring-zinc-200"
                />
              ) : (
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs font-medium text-zinc-500">
                  {connectionStatus.isConnected ? "Connected account" : "TikTok account"}
                </div>
                ...
                {connectionStatus.isConnected ? (
                  <div className="mt-0.5 flex items-center gap-1.5 text-xs text-zinc-500">
                    ...
                    {connectionStatus.profile?.isVerified ? (
                      <span className="rounded-full bg-cyan-50 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-cyan-700">
                        Verified
                      </span>
                    ) : null}
                  </div>
                ...
                <div
                  className={cn(
                    "mt-3 rounded-md border px-2.5 py-1.5 text-xs font-medium",
                    connectionStatus.isConnected
                ...
                {connectionStatus.isConnected ? (
                  <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
                    <AccountStat
                      label="Followers"
                      value={formatProfileCount(
                        connectionStatus.profile?.followerCount
                      )}
                    />
                    <AccountStat
                      label="Following"
                      value={formatProfileCount(
                        connectionStatus.profile?.followingCount
                      )}
                    />
                    <AccountStat
                      label="Likes"
                      value={formatProfileCount(
                        connectionStatus.profile?.likesCount
                      )}
                    />
                  </div>
                ) : null}
...
function formatConnectionExpiry(value?: string) {
  if (!value) {
    return "Session active";
  }

  return `Until ${dateFormatter.format(new Date(value))}`;
}

function formatProfileCount(value?: number, noun?: string) {
  if (typeof value !== "number") {
    return noun ? `${noun} unavailable` : "Unavailable";
  }
...
function getConnectedAccountSummary({
  profile,
  scopes,
}: TikTokConnectionStatus) {
  const grantedStats = scopes?.includes("user.info.stats");
  const grantedProfile = scopes?.includes("user.info.profile");

  if (profile?.videoCount !== undefined) {
    return `Login Kit connected / ${numberFormatter.format(
      profile.videoCount
    )} public videos`;
  }

  if (!grantedStats) {
    return "Login Kit connected / enable user.info.stats for video count";
  }

  if (!grantedProfile) {
    return "Login Kit connected / profile scope not granted";
  }

  return "Login Kit connected";
}
